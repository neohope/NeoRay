...
`long_task` is a lightweight marker. Calling it tells neoray: "this thread has a sustained objective; keep that objective visible across turns and surface it in the UI."
...
1. **Use discovery tools when appropriate** — If the ecosystem is unfamiliar or brittle, **`web_search`**, doc/web fetch **early**—before committing to architecture or rewriting large areas. Narrow queries tied to decisions you must make next.
