...
2. **Call the reflection tool** with the appropriate action
...
| Situation | Command |
|-----------|---------|
| Large codebase analysis | `reflection(action="set", key="context_window_tokens", value=131072)` |
| Repetitive simple tasks | `reflection(action="set", key="model", value="<fast-model>")` |
| Long multi-step task | `reflection(action="set", key="max_iterations", value=80)` |
...
- If `tools.reflection.allow_set` is false, check only

## Related tools

| Need | Use | Persists? |
|------|-----|-----------|
| Per-session temp state | `reflection(action="set", key="...", value=...)` | No |
| Long-term facts | Memory skill (`MEMORY.md`, `USER.md`) | Yes |
| Permanent config change | Edit config file | Yes |

**Rule of thumb:** Tomorrow? Memory. This turn only? Reflection.
